...
public function myfeedback(){
    $data=DB::table('feeback')->join('subject','feeback.ID_monhoc','=','subject.ID_monhoc')->orderBy('feeback.id','DESC')->select('*')->get();
...
public function deletetemyfeedback($ID_myfeedback){
    $student=DB::table('feeback')->where('id',$ID_myfeedback);
    $student->delete();
...
public function approved($id){
  ...
  $data1=DB::select('select id from feeback where  status=?',['No stauts']);

  $data3=DB::select('select id from feeback where  status=?',['canceled']);
   if($data1){
...
public function canceled($id){
    ...
    $data3=DB::select('select id from feeback where  status=?',['approved']);
    if($data1){
